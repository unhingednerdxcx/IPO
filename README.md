...
Sequender combines multiple key UX principles to create an amazing user experience. Upon opening the app for the first time, You will be presented with a login screen. You can choose to log-in using **Google authentication** or **E-mail** 📬. After signing in, you can make new tasks, routine, and even do challenges. These challenges are automatically generated by `generator.py` and these challenges are stored in my apps Firestore, specifically, at `Tasks/Tasks` (which also contain other data, however we will cover that in the *Technical details* section). `generator.py` automatically runs every midnight (thanks to GitHub actions and also check `.github/workflows/schedual.yaml` to learn more about the automation). Going back to the tasks, you can categorize them and the ap will automatically tell you which tasks you have to do today 📋. The relatively simple yet elegant UI keeps you focused on your tasks. **Google**'s material icons and fonts were used to create a native-like UI
